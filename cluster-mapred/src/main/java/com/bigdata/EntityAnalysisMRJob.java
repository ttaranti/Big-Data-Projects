...
public class EntityAnalysisMRJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		/*
		 * TINO:  When running this on the cluster
		 * hadoop jar ubu-mr.jar com.eduonix.hadoop.partone.EntityAnalysisMRJob
		 *     or
		 * yarn jar ubu-mr.jar com.eduonix.hadoop.partone.EntityAnalysisMRJob
		 * 
		 * Provide the relative project root folder for the user running the job as: /user/<user name>/<project root folder>. 
		 * Ex:  /user/oracle/cluster-mapred.
		 */
		if (runOnCluster ) {
			if ( args.length == 1) {
			    projectRootPath = args[0];
			    System.out.println("Running on cluster using projectRootPath: " + projectRootPath);
			}
		   else {
					System.err.println("Incorrect number of arguments.  Expected: <class> <projectRootPath>: " + args[0]);
					return -1;
				}
		}
